...
public:
        ...
        GlutWindow(Window::Parameters().size(1024, 768).depth(true)), fov(80.0), alpha(135), theta(45), dist(15)
    {
       ...
       // resLoader->addPath(dir + "/scenes");
        ...
        light->setLocalToParent(mat4d::translate(vec3d(7, 3, 3)) * mat4d::rotatez(135) * mat4d::rotatex(45));
        light->addFlag("light");
       ...
       ptr<SceneNode> cube = new SceneNode();
        ...
        cube->addModule("material", resManager->loadResource("texturedPlastic").cast<Module>());
        cube->addMethod("draw", new Method(resManager->loadResource("meshMethod").cast<TaskFactory>()));
        root->addChild(cube);
        ...
        plane->addMethod("draw", new Method(resManager->loadResource("meshMethod").cast<TaskFactory>()));
        root->addChild(plane);
